...
void mf(int ny, int nx, int hy, int hx, const float *in, float *out) {
  ...
  // initialize the variables and the sliding window
  ...
  std::vector<float> window((2 * hx + 1) * (2 * hy + 1));
    ...
    if (y + hy >= ny) j_max = ny; else j_max = y + hy + 1;
      ...
      for (int x = 0; x < nx; x++) {
        ...
        // the sliding window may contain values from previous iteration
        // but they are exluded when using bth_element algorithm with limits
